...
class TextCLIP(nn.Module):
    ...
    def forward(self, tgt_input):
        ...
        output = txt_logits[
            torch.arange(txt_logits.shape[0]), tgt_input["input_ids"].argmax(dim=-1)
        ]
...
class ImageCLIP(nn.Module):
    ...
    def forward(self, src_input):
        ...
        attention_mask = F.pad(
            attention_mask.flatten(1), (1, 0), value=1.0
        )  # [b, 64] --> [b, 65]

        outs = self.trans_encoder(
            inputs_embeds=x, attention_mask=attention_mask.cuda(), return_dict=True
        )
...
class Text_Decoder(nn.Module):
    ...
    def forward(self, tgt_input, masked_tgt_input, model_txt):
        ...
        decoder_input_ids = shift_tokens_right(
            tgt_input["input_ids"].cuda(), self.text_decoder.config.pad_token_id
        )
    ...
    if decoder_type == "LD":
        return MBartForConditionalGeneration.from_pretrained(
            ...
            config=AutoConfig.from_pretrained(
                Path(config["model"]["visual_encoder"]) / "config.json"
            ),
        )
        ...
        return MBartForConditionalGeneration.from_pretrained(
            ...
            ignore_mismatched_sizes=True,
            config=AutoConfig.from_pretrained(
                Path(config["model"]["transformer"]) / "LLMD_config.json"
            ),
        )
...
class gloss_free_model(nn.Module):
    def __init__(self, config, args, embed_dim=1024, pretrain=None):
        ...
        if config["model"]["sign_proj"]:
            self.sign_emb = V_encoder(
                emb_size=embed_dim, feature_size=embed_dim, config=config
            )
            self.embed_scale = (
                math.sqrt(embed_dim) if config["training"]["scale_embedding"] else 1.0
            )
        else:
