...
class STCAdapter(nn.Module):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__()
        self.stc_connector = STCConnector(*args, **kwargs)
        self.downsample = self.stc_connector.downsample
    ...
    def forward(self, x, v_length):
        ...
        x = self.stc_connector(x)
        v_length = torch.floor((v_length + 1) / self.downsample[0] + 1)
        return x, v_length
